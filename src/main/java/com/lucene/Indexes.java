...
// Module名字 -> Sources -> Language Level->选个默认的就行。


public class Indexes {
    private IndexWriter writer;

    Indexes(String indexDir) throws Exception{
    ...
    public static void main(String[] args) {
        String indexDir = "E:\\javacode\\lucene\\data"; //将索引保存到的路径
        String dataDir = "E:\\javacode\\lucene\\data"; //需要索引的文件数据存放的目录
        Indexes indexer = null;
        int indexedNum = 0;
        long startTime = System.currentTimeMillis(); //记录索引开始时间
        try {
            indexer = new Indexes(indexDir);
